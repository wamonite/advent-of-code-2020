...
class TileGrid:
    ...
    def _check_tile(self, tile_l, check_dir, tile_r):
        for conn_l_num, conn_r_num, conn_list in self._connection_list:
            if conn_l_num == tile_l[0] and conn_r_num == tile_r[0]:
                for conn_p_l, conn_p_r, conn_dir in conn_list:
                    if tile_l[1] == conn_p_l and tile_r[1] == conn_p_r and check_dir == conn_dir:
                        return True
    ...
    def _check_map_pos(self, map_list, free_tiles, x, y):
        ...
        for selected_num in [num for num in self.connection_count[coord_connections] if num in free_tiles]:
            ...
            for pos in range(POS_MAX):
                tile_above = None if y == 0 else map_list[x + (y - 1) * self.dimension]
                tile_left = None if x == 0 else map_list[(x - 1) + y * self.dimension]
                tile_test = (selected_num, pos)
                try:
                    if tile_above:
                        if not (
                            self._check_tile(tile_above, 'above', tile_test) or
                            self._check_tile(tile_test, 'below', tile_above)
                        ):
                            raise ValueError(f'tile above not found {x} {y}')

                    if tile_left:
                        if not (
                            self._check_tile(tile_left, 'left', tile_test) or
                            self._check_tile(tile_test, 'right', tile_left)
                        ):
                            raise ValueError(f'tile left not found {x} {y}')

                    new_map = list(map_list)
                    new_map[x + y * self.dimension] = tile_test
